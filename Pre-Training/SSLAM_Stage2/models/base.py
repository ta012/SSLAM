...
class ModalitySpecificEncoder(nn.Module):
    ...
    def local_features(self, features):
        ...
        assert "PatchEmbed_new" in self.local_encoder.__class__.__name__, "local encoder is not PatchEmbed_new"

        x = self.project_features(x)
        assert isinstance(self.project_features,nn.Identity)
        return x
    ...
    def contextualized_features(
        ...
        if self.fixed_positional_encoder is not None:
            # print("fixed pos embdeing is data indepenendt(x=None) and non trainiable, this assert pass ", torch.equal(self.fixed_positional_encoder(x, padding_mask),self.fixed_positional_encoder(x=None, padding_mask=None)))
            x = x + self.fixed_positional_encoder(x, padding_mask)[:,:x.size(1),:]
        else:
            exit("why here? self.fixed_positional_encoder is None")

        if mask:
            if clone_batch > 1:
                if VERBOSE:
                    print("repeat interleave inp :",x.shape,clone_batch)
                x = x.repeat_interleave(clone_batch, 0) ## tensor([1, 2, 3]) -> tensor([1, 1, 2, 2, 3, 3])
                if VERBOSE:
                    print("repeat interleave out :",x.shape,clone_batch)
            ...
            if VERBOSE:
                print("compute mask inp ",x.shape)
            ...
            if VERBOSE:
                print("compute mask out ",x.shape)

        if self.relative_positional_encoder is not None:
            exit("why here self.relative_positional_encoder")
            if VERBOSE:
                print("relative_positional_encoder inp shape:",self.relative_positional_encoder)
        ...
        if self.get_alibi_bias is not None:
            print('get_alibi_bias   ',self.get_alibi_bias)
            exit('why here get_alibi_bias')
        ...
        if self.extra_tokens is not None:
            ...
            if alibi_bias is not None:
                exit('why here alibi_bias')
                # B x H x T x T
                alibi_bias = F.pad(alibi_bias, (num, 0, num, 0))

        if VERBOSE:
            print("Contextualized features inp shape:",x.shape)
        ...
        if VERBOSE:
            print("Contextualized features out shape:",x.shape)
    ...
    def forward(
        ...
        if VERBOSE:
            print("Local features inp and out shape:",features.shape, x.shape)
