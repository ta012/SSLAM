...
DATA_STATS = {'mean': -5.275 , 'std': 3.268} ### RunAll -5.274658 3.267791
print("\nUsing new normalization stats DATA_STATS: ", DATA_STATS)
class FileAudioDataset(RawAudioDataset):
    ...
    def __getitem__(self, index):
        ...
        while wav is None: ## dont except any wav for fail but still 
            try:
                
                wav, curr_sample_rate = sf.read(path_or_fp, dtype="float32")
            except:
                exit('Failed to read {}. Exiting...'.format(path_or_fp))
                # exit("Failed to read {}. Exiting...".format(path_or_fp))
                print("Failed to read {}. Retrying... with new idx".format(path_or_fp))
                new_idx = np.random.randint(0, len(self.fnames)-1)
                fn = self.fnames[new_idx]
                fn = fn if isinstance(self.fnames, list) else fn.as_py()
                fn = self.text_compressor.decompress(fn)
                path_or_fp = os.path.join(self.root_dir, fn)
                _path, slice_ptr = parse_path(path_or_fp)
        ...
        if self.wav2fbank:
            ...
            ### use new normalization stats everything else AS 
            self.norm_mean = DATA_STATS['mean']
            self.norm_std =  DATA_STATS['std']
