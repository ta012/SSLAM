...
eat_dirname=AS2M/EAT_FtCkptMixRegionExtraMixLCls1Layer2S2TDrpInpTBothNoMixClsAS2M
...
${mnt_vol}/eat_apr30/fairseqvenv/bin/python ${main_dir}/fairseq_cli/hydra_train.py -m \
    ...
    task.data=${mnt_vol}/eat_data_manifest/manifest_as2mfull_ft \
    task.h5_format=false \
    task.num_samples=200000 \
    task.AS2M_finetune=true \
    task.weights_file=${mnt_vol}/eat_data_manifest/manifest_as2mfull_ft/merged_unbal_bal_train_weka_weight.csv \
